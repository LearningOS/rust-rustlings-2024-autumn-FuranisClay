...
mod Foo {
    #[no_mangle]  // 确保函数不被符号改编
    pub fn my_demo_function(a: u32) -> u32 {
        a
    }

    // 创建一个别名
    #[no_mangle]  // 确保别名也不被符号改编
    pub fn my_demo_function_alias(a: u32) -> u32 {
        my_demo_function(a)  // 调用原始函数
    }
}
...
mod tests {
    ...
    fn test_success() {
        ...
        // Rust function.
        unsafe {
            assert_eq!(my_demo_function(123), 123);        // 验证调用的结果
            assert_eq!(my_demo_function_alias(456), 456);  // 验证调用的结果
        }
    }
}
